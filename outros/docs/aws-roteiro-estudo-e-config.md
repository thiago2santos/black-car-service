...
Este roteiro foca em uma alternativa AWS para hospedar a landing e preparar a camada de API usando o mínimo de serviços possível para validar o MVP.

Observação:

- hoje a implementação oficial da landing está consolidada em WordPress
- este roteiro permanece como alternativa arquitetural futura, especialmente para API, CDN e desacoplamento
...
### 2. Hospedar uma versão estática da landing no S3

> Esta seção passou a ser uma alternativa arquitetural.
> Hoje a fonte oficial da landing está no WordPress.
> Use este bloco apenas se quiser voltar a uma estratégia de frontend estático.
...
3. **Upload da landing**
   - Subir um frontend estático preparado para esse fluxo, com `index.html` e seus assets correspondentes.

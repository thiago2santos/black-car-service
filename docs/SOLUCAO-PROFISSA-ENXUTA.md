...
- **Front (landing page)**  
  - `index.html` + `assets/` hospedados como **site estático**.
...
- **Hospedagem alvo**:
  - **S3** (static website hosting) servindo os arquivos estáticos.
  - **CloudFront** na frente do S3:
    - HTTPS com certificado do **AWS Certificate Manager (ACM)**.
    - Domínio próprio (ex.: `https://blackcarsservice.com`).
  - DNS (Route 53 ou outro) apontando o domínio para o CloudFront.

- **Funcionalidades da landing**:
  - UI escura (slate) com destaques em âmbar (amber).
  - Seletor de idiomas (PT / EN / ES) com JS de i18n (`assets/js/lang.js`).
  - Formulário de cotação com:
    ...
    - Autocomplete de origem e destino (`assets/js/destination-autocomplete.js`) usando Nominatim.
  - Banner de feedback:
...
- **Arquivo**: `scripts/google-sheets-webapp.gs`
- **Fluxo**:
